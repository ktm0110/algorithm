...
bool is_collision_free(const PointF& p1, const PointF& p2, int width, int height, const vector<vector<int>>& grid) {
    ...
    double step_size)
{
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_real_distribution<> dis_w(0, width - 1);
    uniform_real_distribution<> dis_h(0, height - 1);
    uniform_real_distribution<> dis_goal(0, 1);
    ...
    for (int i = 0; i < max_iter; ++i) {
        ...
        vector<int> near_node_ids;
        double search_radius = min(step_size * 3.0, 50.0);
        for (int j = 0; j < tree.size(); ++j) {
    ...
    if (goal_node_id == -1) {
        double min_dist_to_goal = numeric_limits<double>::max();
        int best_node_id = -1;
        for (size_t i = 0; i < tree.size(); ++i) {
